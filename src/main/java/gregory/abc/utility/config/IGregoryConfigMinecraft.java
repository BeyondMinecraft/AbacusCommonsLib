...
/**
 * A abstract public class that defines configuration fields and methods to handle loading and saving your settings.<p/>
 * For each of the fields and methods that go with this interface definition, I have supplied some sample code to start.
 * Each method body can and should be customized to handle the configuration data that go with this class.<p/>
 ...
 * </pre>
 * In the above sample, I use the sample class name of "MyModConfig"; in your case, replace those instances with the
 * name of your configuration class. The "quit" methods should ensure any open configuration files and objects are
...
abstract public class IGregoryConfigMinecraft {
   ...
   * This way, you do not "pollute" other world-saves with irrelevant information that need not be loaded.
   * (This can save your sanity, and that of your mod users&hellip;)
   * @see Configuration Configuration
   * @see IGregoryConfigMinecraft IGregoryConfigMinecraft
   * @see #configGlobal configGlobal
   * @see #configWorldZ configWorldZ
  ...
  /**
   * A public {@link File} object used as the location of the {@link Minecraft#mcDataDir Minecraft data directory}.<p/>
  ...
  /**
   * A "helper" method for initializing the global configuration file.<p/>
   * This method is useful for when only globally-relevant configurations are necessary. Only one configuration object
   * needs to be loaded and maintained; the other file objects are left as {@code null} value, but can be set and loaded
   * as necessary. Also, the global configuration only should ever need to be loaded once per Minecraft playing session;
   * although it could be possible to retrofit your implementation of this class to be able to reload configurable data,
   * your code could be better written so that it is not necessary.<p/>
   * Start with this template code, and modify it to suit your needs:
   * <pre>
   ...
   *    }</pre>
   * Because of the "createNewFile()" method call above, you will either have to handle the thrown exception or modify
   * the method signature to throw the exception to the calling method, where it can be handled externally.
   * (I would prefer it handled within this configuration class; but it is your coding methodology that matters.)
  ...
  /**
   * Load all information from the global configuration file.<p/>
   * This method is public so that if the mod requires re-loading information (some kind of "reset")
   * it is easy to retrieve a "last known configuration" for global settings.<p/>
   * Start with this template code, and modify it to suit your needs:
   * <pre>
   ...
   *
   *    saveConfigGlobal(); // Ensure the config object is saved after successfully loading.</pre>
   * @see IGregoryConfigMinecraft IGregoryConfigMinecraft
   * @see #configGlobal configGlobal
   * @see #initConfigGlobal() initConfigGlobal()
   * @see #quitConfigGlobal() quitConfigGlobal()
  ...
  /**
   * Load all information from the world configuration file.<p/>
   * This method is public so that if the mod requires re-loading information (some kind of "reset")
   * it is easy to retrieve a "last known configuration" for world-specific settings.<p/>
   * Start with this template code, and modify it to suit your needs:
   * <pre>
   ...
   *
   *    saveConfigWorld(); // Ensure the config object is saved after successfully loading.</pre>
   * @see IGregoryConfigMinecraft IGregoryConfigMinecraft
   * @see #configWorld configWorld
   * @see #initConfigWorld() initConfigWorld()
   * @see #quitConfigWorld() quitConfigWorld()
  ...
  /**
   * Load all information from the dimension configuration file.<p/>
   * This method is public so that if the mod requires re-loading information (some kind of "reset")
   * it is easy to retrieve a "last known configuration" for dimension-specific settings.<p/>
   * Start with this template code, and modify it to suit your needs:
   * <pre>
   ...
   *
   *    saveConfigWorldZ(); // Ensure the config object is saved after successfully loading.</pre>
   * @see IGregoryConfigMinecraft IGregoryConfigMinecraft
   * @see #configWorldZ configWorldZ
   */
  ...
  /**
   * Save all information to the global configuration file.<p/>
   * This method is public so that if the mod requires re-saving global information (after a change)
   * it is easy to do so with a single method call.<p/>
   * Start with this template code, and modify it to suit your needs:
   * <pre>
   *    configGlobal.save();
   *    quitConfigGlobal(); // Optional; to release memory no longer needed.</pre>
   * @see IGregoryConfigMinecraft IGregoryConfigMinecraft
   * @see #configGlobal configGlobal
   * @see #initConfigGlobal() initConfigGlobal()
   * @see #loadConfigGlobal() loadConfigGlobal()
  ...
  /**
   * Save all information to the world configuration file.<p/>
   * This method is public so that if the mod requires re-saving any per-world information (after a change)
   * it is easy to do so with a single method call.<p/>
   * Start with this template code, and modify it to suit your needs:
   * <pre>
   *    configWorld.save();
   *    quitConfigWorld(); // Optional; to release memory no longer needed.</pre>
   * @see IGregoryConfigMinecraft IGregoryConfigMinecraft
   * @see #configWorld configWorld
   * @see #initConfigWorld() initConfigWorld()
   * @see #loadConfigWorld() loadConfigWorld()
  ...
  /**
   * Save all information to the dimension configuration file.<p/>
   * This method is public so that if the mod requires re-saving any per-world information (after a change)
   * it is easy to do so with a single method call.<p/>
   * Start with this template code, and modify it to suit your needs:
   * <pre>
   *    configWorldZ.save();
   *    quitConfigWorldZ(); // Optional; to release memory no longer needed.</pre>
   * @see IGregoryConfigMinecraft IGregoryConfigMinecraft
   * @see #configWorldZ configWorldZ
   */
  ...
  /**
   * A public setter method to set the precise location of the {@link #configWorld configWorld} file object.<p/>
   * That object allows for per-world configurations, in that a player can have specific settings for each world that is
   * being loaded; this makes some things, like radars that can set waypoints and death markers, more specific.
   * Initialization of the {@link #fileWorld fileWorld} (and any relevant file-checking) should go here.<p/>
   * Start with this template code, and modify it to suit your needs:
   * <pre>
   *    fileWorld = f;
   *    //TODO: Perhaps, check to see if it exists?</pre>
  ...
  /**
   * A public setter method to set the precise location of the {@link #configWorldZ configWorldZ} file object.<p/>
   * That object allows for per-dimension configurations, which is not necessary for most mods; however, in the off-hand
   * chance a mod may want to allow configurations for things based upon the current dimension, initialization of the
   * {@link #fileWorldZ fileWorldZ} (and any relevant file-checking) should go here.<p/>
   * Start with this template code, and modify it to suit your needs:
   * <pre>
   *    fileWorldZ = f;
   *    //TODO: Perhaps, check to see if it exists?</pre>
